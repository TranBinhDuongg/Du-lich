(() => {
  ...
  function destinationLabel(input) {
    const names = destinationIds(input).map((id) => E.destinations[id].name);
    return names.length > 3
      ? names.slice(0, 3).join(", ") + " +" + (names.length - 3) + " điểm"
      : names.join(", ");
  }
  function dayDestinationLabel(day, input) {
    const ids = [
      ...new Set(
        day.activities
    ...
    return (ids.length ? ids : destinationIds(input))
  ...
  function show() {
    try {
      const id = new URLSearchParams(location.search).get("id"),
        records = JSON.parse(
          localStorage.getItem("tripmate.saved-plans.v1") || "[]",
        );
      const p = Array.isArray(records)
        ? records.find((p) => p.id === id)
        : null;
      if (!p)
        throw Error(
          "Không tìm thấy lịch trình. Lịch trình chỉ được lưu trên trình duyệt này và có thể đã bị xóa.",
        );
      E.validate(p.input);
      if (
        ...
        throw Error("Dữ liệu lịch trình không hợp lệ.");
      const d = E.destinations[p.input.destination],
        c = E.costs(p);
      ...
      $("route-days").innerHTML = p.days
        .map(
          (day, i) =>
            '<a href="#route-day-' + (i + 1) + '">Ngày ' + (i + 1) + "</a>",
        )
        .join("");
      $("route-path").innerHTML =
        p.days
          .map(
            (day, i) =>
              '<section class="route-day" id="route-day-' +
              (i + 1) +
              '"><header><span class="day-marker">' +
              String(i + 1).padStart(2, "0") +
              "</span><div><p>CHẶNG HÀNH TRÌNH</p><h2>Ngày " +
              (i + 1) +
              " · " +
      ...
      if (photo) {
        ...
        $("route-photo").alt = destinationLabel(p.input);
        $("route-photo").hidden = false;
      }
        ...
        $(button).onclick = () => {
          try {
            sessionStorage.setItem(
              "tripmate.current-plan.v1",
              JSON.stringify({ plan: p, dirty: false }),
            );
            window.TripMateNavigate(target);
          } catch {
            $("route-error").textContent =
              "Không thể mở bản chỉnh sửa vì trình duyệt không cho lưu bản nháp.";
          }
        };
    } catch (e) {
      $("route-content").hidden = true;
      $("route-title").textContent = "Chưa thể mở hành trình";
  ...
  function drawCurves() {
    ...
    document.querySelectorAll(".route-day ol").forEach((list) => {
      ...
      if (!box.width) return;
      const points = [...list.querySelectorAll(".stop-marker")].map((el) => {
        ...
        return {
          ...
          bottom: card.bottom - box.top,
        };
      });
      if (points.length < 2) return;
      const svg = document.createElementNS(ns, "svg");
      svg.classList.add("route-curve");
      ...
      for (const cls of ["route-curve-base", "route-curve-dashes"]) {
  ...
  document
    .getElementById("route-path")
    .addEventListener("load", scheduleCurves, true);
  let curveFrame;
  function scheduleCurves() {
    cancelAnimationFrame(curveFrame);
    curveFrame = requestAnimationFrame(drawCurves);
  }
  new ResizeObserver(scheduleCurves).observe(
  ...
  window.addEventListener("storage", (e) => {
    if (e.key === "tripmate.saved-plans.v1") {
      show();
      scheduleCurves();
    }
  });
