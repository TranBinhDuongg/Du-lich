...
document.querySelectorAll('[data-open-chat], [data-prompt]').forEach(button => button.addEventListener('click', openChat));
document.querySelector('.close-chat').addEventListener('click', closeChat);
dialog.addEventListener('click', event => { if (event.target === dialog) { const r = dialog.getBoundingClientRect(); if(event.clientX < r.left || event.clientX > r.right || event.clientY < r.top || event.clientY > r.bottom) closeChat(); } });
...
document.addEventListener('keydown', event => { if (event.key === 'Escape' && !menu.hidden) { menu.hidden = true; menuButton.setAttribute('aria-expanded', 'false'); menuButton.focus(); } });
...
const move = direction => journey.scrollBy({ left: direction * journey.clientWidth * .75, behavior: scrollBehavior() });
...
journey.addEventListener('wheel', event => {
  ...
  event.preventDefault();
  journey.scrollLeft += event.deltaY * (event.deltaMode === 1 ? 20 : event.deltaMode === 2 ? journey.clientWidth : 1);
}, { passive: false });
let drag;
...
journey.addEventListener('pointermove', event => { if (drag) journey.scrollLeft = drag.scroll - (event.clientX - drag.x); });
function stopDrag() { drag = null; journey.classList.remove('dragging'); }
...
updateProgress();
document.querySelectorAll('[data-hotel]').forEach(button => button.addEventListener('click', () => {
  const image = document.getElementById(button.dataset.hotel);
  image.src = button.dataset.image;
  image.alt = `${button.textContent} — lưu trú tham khảo`;
  button.parentElement.querySelectorAll('button').forEach(option => { option.classList.toggle('selected', option === button); option.setAttribute('aria-pressed', String(option === button)); });
}));
...
(() => {
  ...
  const scenes = [
  {
    ...
    "description": "Vươn ra biển giữa những đảo đá.<br>Cảnh đẹp còn đó, hành trình đang chờ."
  },
  {
    "name": "Bản Giốc",
    "image": "assets/images/ban-gioc.png",
    "bg": "#254b41",
    "ink": "#e1efc8",
  ...
  {
    "name": "Cầu Vàng",
    "image": "assets/images/cau-vang.jpg",
    "bg": "#355a55",
    "ink": "#e2efce",
  ...
  {
    "name": "Lý Sơn",
    "image": "assets/images/ly-son.jpg",
    "bg": "#174d63",
    "ink": "#c7e9e9",
  ...
  let current = 0, desired = 0, busy = false, sequence = 0, active = true;
  let transition = null, frame = 0, last = 0, wheelTotal = 0, wheelTime = 0, lastSwitch = 0;
  let drag = null, pointer = [0,0], smoothPointer = [0,0], gesture = 0;
  ...
  function initGL() {
    ...
    try{
      ...
      if(!gl.getProgramParameter(program,gl.LINK_STATUS))throw Error('Original shader link failed');
      gl.useProgram(program);
      // Original Travelshift geometry: PlaneGeometry(1, 1, 40, 40).
      const vertices=[];
      for(let y=0;y<40;y++)for(let x=0;x<40;x++){
        for(const [dx,dy] of [[0,0],[1,0],[0,1],[0,1],[1,0],[1,1]]){
          const u=(x+dx)/40,v=(y+dy)/40;
          vertices.push(u-.5,v-.5,0,u,v);
        }
      }
      ...
      uniforms={};
      ['projectionMatrix','modelViewMatrix','deformation','percentVisible','resolution','time','shapeIndex','texture','texture_size','texture_imageSize','texture_alpha','texture_rotation'].forEach(k=>uniforms[k]=gl.getUniformLocation(program,k));
      gl.uniform1i(uniforms.texture,0);
      gl.pixelStorei(gl.UNPACK_FLIP_Y_WEBGL,true);
      gl.enable(gl.BLEND);gl.blendFuncSeparate(gl.SRC_ALPHA,gl.ONE_MINUS_SRC_ALPHA,gl.ONE,gl.ONE_MINUS_SRC_ALPHA);
      images.forEach(asset=>asset.ready.then(img=>{if(img&&gl&&!disposed){try{upload(asset);wake();}catch(error){console.error('TripMate texture:',error);canvas.dataset.error=error.message;useFallback();}}}));
    }catch(error){console.error('TripMate WebGL:',error);canvas.dataset.error=error.message;useFallback();}
  }


  function useFallback(){gl=null;art.classList.remove('webgl-ready');}
  function upload(asset){
    if(!gl||!asset.img.naturalWidth||asset.texture)return;
    const texture=gl.createTexture();gl.bindTexture(gl.TEXTURE_2D,texture);
    gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MIN_FILTER,gl.LINEAR);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_MAG_FILTER,gl.LINEAR);
    gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_S,gl.CLAMP_TO_EDGE);gl.texParameteri(gl.TEXTURE_2D,gl.TEXTURE_WRAP_T,gl.CLAMP_TO_EDGE);
    gl.texImage2D(gl.TEXTURE_2D,0,gl.RGBA,gl.RGBA,gl.UNSIGNED_BYTE,asset.img);asset.texture=texture;
  }

  function draw(progress=0,from=current,to=current,direction=1,deformOverride=null){
    if(!gl||motion.matches||!active)return;
    const a=images[from],b=images[to];
    if(!a.texture||!b.texture){art.classList.remove('webgl-ready');return;}
    ...
    const cw=art.clientWidth,ch=art.clientHeight,near=.1,far=2000;
    gl.uniformMatrix4fv(uniforms.projectionMatrix,false,new Float32Array([200/cw,0,0,0,0,200/ch,0,0,0,0,-(far+near)/(far-near),-1,0,0,-2*far*near/(far-near),0]));
    gl.uniformMatrix4fv(uniforms.modelViewMatrix,false,new Float32Array([cw,0,0,0,0,ch,0,0,0,0,1,0,0,0,-100,1]));
    gl.uniform2f(uniforms.resolution,cw,ch);
    gl.uniform1f(uniforms.deformation,deformOverride===null?Math.sin(progress*Math.PI):deformOverride);
    ...
    function plane(asset,index,visible){
      gl.activeTexture(gl.TEXTURE0);gl.bindTexture(gl.TEXTURE_2D,asset.texture);
      gl.uniform2f(uniforms.texture_size,asset.img.naturalWidth,asset.img.naturalHeight);
      gl.uniform2f(uniforms.texture_imageSize,asset.img.naturalWidth,asset.img.naturalHeight);
    ...
    if(from===to)plane(a,from,1.001);
      ...
      plane(a,from,1+Math.min(progress,.999));
  ...
  function wake(){if(!frame&&active&&!disposed&&!document.hidden)frame=requestAnimationFrame(tick);}

  let originalHold=null,originalSettle=null,originalPosition=0,originalDeformation=0;
  const quartInOut=t=>t<.5?8*t*t*t*t:1-8*Math.pow(t-1,4);
  ...
  function startOriginalHold(){
    ...
    originalSettle=null;originalPosition=current;originalDeformation=0;
    originalHold={start:performance.now()};last=0;wake();return true;
  }
  function releaseOriginalHold(){
    if(!originalHold)return;
    originalHold=null;
    const base=Math.floor(originalPosition),fraction=originalPosition-base;
    const end=base+(fraction>.3?1:0);
    originalSettle={start:performance.now(),from:originalPosition,to:end+.001,deformation:originalDeformation};
    wake();
  }
  function drawOriginalPosition(){
    const base=Math.floor(originalPosition),fraction=originalPosition-base;
    const from=wrap(base),to=wrap(base+1);
    draw(fraction,from,to,1,originalDeformation);
    const nearest=wrap(Math.round(originalPosition));
    if(current!==nearest){current=nearest;desired=current;updateCopy(current,1);fallback.src=scenes[current].image;}
  }
  function tick(now){
    frame=0;
    if(!active||disposed||document.hidden)return;
    const dt=last?Math.min(now-last,48):16;last=now;
    if(originalHold){
      const elapsed=(now-originalHold.start)/1000;
      originalDeformation=quartInOut(clamp(elapsed/.8,0,1));
      const speed=Math.pow(clamp(elapsed/.4,0,1),4);
      const base=Math.floor(originalPosition);
      if(images[wrap(base)].texture&&images[wrap(base+1)].texture||!gl){
      ...
      originalPosition=originalSettle.from+(originalSettle.to-originalSettle.from)*quartOut(t);
      originalDeformation=originalSettle.deformation*(1-quartOut(relax));
      drawOriginalPosition();
      if(relax>=1){originalSettle=null;originalDeformation=0;draw();}
    }else if(transition){
      const t=motion.matches?1:clamp((now-transition.start)/transition.duration,0,1);
      draw(quartInOut(t),transition.from,transition.to,transition.direction);
      if(t>=1){const done=transition.done;transition=null;done();}
    }else draw();
  ...
  function setTitle(name,direction=1){
    title.getAnimations({subtree:true}).forEach(animation=>animation.cancel());
    title.textContent='';title.setAttribute('aria-label',name+'.');
    Array.from(name+'.').forEach((letter,i)=>{
      const span=document.createElement('span');span.className='glyph';span.textContent=letter;span.setAttribute('aria-hidden','true');title.append(span);
      if(!motion.matches&&span.animate)span.animate([{opacity:0,transform:'translate3d('+direction*45+'px,65%,0) rotate(5deg)'},{opacity:1,transform:'translate3d(0,0,0) rotate(0deg)'}],{duration:1000,delay:i*38,easing:'cubic-bezier(.16,1,.3,1)',fill:'backwards'});
    });
  }
  function updateCopy(index,direction){
    const scene=scenes[index];
    document.body.style.setProperty('--scene-bg',scene.bg);document.body.style.setProperty('--scene-ink',scene.ink);
    ...
    document.getElementById('home-region').textContent='VIỆT NAM / '+scene.name.toLocaleUpperCase('vi-VN');
    document.getElementById('home-counter').textContent=String(index+1).padStart(2,'0')+' / '+String(scenes.length).padStart(2,'0');
    document.getElementById('home-explore').firstChild.textContent=index===0?'Khám phá hành trình ':'Hỏi về điểm đến ';
    dots.forEach((dot,i)=>dot.setAttribute('aria-pressed',String(i===index)));
    status.textContent='Điểm đến '+scene.name;
    const copy=home.querySelector('.home-description');
    if(!motion.matches&&copy.animate)copy.animate([{opacity:0,transform:'translateY(18px)'},{opacity:1,transform:'translateY(0)'}],{duration:850,easing:'cubic-bezier(.16,1,.3,1)'});
  }
  async function switchScene(index,direction=1){
    originalHold=null;originalSettle=null;
    desired=(index+scenes.length)%scenes.length;
    if(busy||desired===current)return;
    busy=true;const target=desired;const token=++sequence;
    await images[target].ready;
    if(disposed||token!==sequence){busy=false;return;}
    updateCopy(target,direction);
    fallback.src=scenes[target].image;
    if(!gl&&!motion.matches&&fallback.animate)fallback.animate([{opacity:0,transform:'scale(.8) rotate('+(-direction*5)+'deg)'},{opacity:1,transform:'scale(.88) rotate(0)'}],{duration:1000,easing:'cubic-bezier(.22,1,.36,1)'});
    const finish=()=>{current=target;busy=false;draw();if(desired!==current)switchScene(desired,direction);};
    if(motion.matches||!active){finish();return;}
    transition={from:current,to:target,direction,start:performance.now(),duration:1150,done:finish};wake();
  }
  function next(direction){lastSwitch=performance.now();switchScene(desired+direction,direction);}
  function showHome(){
    active=true;home.hidden=false;journey.hidden=true;journey.inert=true;footer.hidden=true;
    document.body.classList.add('is-home');menu.hidden=true;menuButton.setAttribute('aria-expanded','false');
    wake();
  }
  function showJourney(){
    originalHold=null;originalSettle=null;
    active=false;home.hidden=true;journey.hidden=false;journey.inert=false;footer.hidden=false;
    document.body.classList.remove('is-home');
    if(transition){const done=transition.done;transition=null;done();}
    if(frame)cancelAnimationFrame(frame);frame=0;
    journey.focus({preventScroll:true});window.dispatchEvent(new Event('resize'));
  }
  document.querySelector('.wordmark').addEventListener('click',event=>{event.preventDefault();event.stopImmediatePropagation();showHome();home.focus({preventScroll:true});},true);
  menu.querySelectorAll('a').forEach(link=>link.addEventListener('click',showJourney,true));
  document.getElementById('home-explore').addEventListener('click',()=>{
    if(current===0){showJourney();return;}
    openChat();sendMessage('Mình muốn khám phá '+scenes[current].name+'.');
  });
  document.getElementById('home-prev').addEventListener('click',()=>next(-1));
  document.getElementById('home-next').addEventListener('click',()=>next(1));
  document.getElementById('home-shuffle').addEventListener('click',()=>switchScene(desired+1+Math.floor(Math.random()*(scenes.length-1))));
  dots.forEach(dot=>dot.addEventListener('click',()=>switchScene(Number(dot.dataset.slide),Number(dot.dataset.slide)>=current?1:-1)));
  ...
  function holdScene(button, action) {
    let held=false,timer=null,suppressClick=false,startX=0,startY=0;
    const cancel=()=>{
      clearTimeout(timer);timer=null;
      if(held)releaseOriginalHold();
      held=false;button.classList.remove('hover-armed');
    };
    holdCancels.push(cancel);
    button.addEventListener('pointerdown',event=>{
      if(event.button!==0||!event.isPrimary||!active||dialog.open)return;
      if(button===home&&event.target.closest('button,a,input,textarea,select'))return;
      cancel();suppressClick=true;startX=event.clientX;startY=event.clientY;
      button.setPointerCapture(event.pointerId);
      button.style.setProperty('--hold-x',startX+'px');button.style.setProperty('--hold-y',startY+'px');
      if(motion.matches||!gl){
        held=true;timer=setTimeout(()=>{cancel();if(active&&!dialog.open)action();},600);
      }else held=startOriginalHold();
      if(held)button.classList.add('hover-armed');
    });
    button.addEventListener('pointermove',event=>{
      if(held&&Math.hypot(event.clientX-startX,event.clientY-startY)>14)cancel();
    });
    ['pointerup','pointercancel','lostpointercapture','pointerleave','blur'].forEach(type=>button.addEventListener(type,cancel));
  ...
  home.querySelector('.home-instruction').textContent = 'Nhấn giữ chuột trái để đổi điểm đến';
  ...
  holdScene(shuffle, () => switchScene(desired + 1 + Math.floor(Math.random() * (scenes.length - 1))));
  document.addEventListener('visibilitychange', () => { if (document.hidden) holdCancels.forEach(cancel => cancel()); });
  window.addEventListener('pagehide', () => holdCancels.forEach(cancel => cancel()));
  home.addEventListener('wheel',event=>{
    if(event.ctrlKey||event.target.closest('button'))return;
    event.preventDefault();
    const now=performance.now();
    if(now-lastSwitch<900)return;
    if(now-wheelTime>160)wheelTotal=0;wheelTime=now;
    const delta=Math.abs(event.deltaX)>Math.abs(event.deltaY)?event.deltaX:event.deltaY;
    wheelTotal+=delta*(event.deltaMode===1?20:1);
    if(Math.abs(wheelTotal)>45){next(Math.sign(wheelTotal));wheelTotal=0;}
  },{passive:false});
  home.addEventListener('keydown',event=>{
    if(event.target.closest('button'))return;
    if(event.key==='ArrowRight'||event.key==='ArrowDown'){event.preventDefault();next(1);}
    if(event.key==='ArrowLeft'||event.key==='ArrowUp'){event.preventDefault();next(-1);}
  });
  home.addEventListener('pointerdown',event=>{
    if(event.target.closest('button,a')||event.button!==0)return;
    drag={x:event.clientX,y:event.clientY,id:event.pointerId};home.setPointerCapture(event.pointerId);home.classList.add('is-dragging');
  });
  home.addEventListener('pointermove',event=>{
    if(motion.matches)return;
    if(finePointer.matches){pointer=[clamp((event.clientX/innerWidth-.5)*2,-1,1),clamp((event.clientY/innerHeight-.5)*2,-1,1)];}
    if(drag)gesture=clamp((event.clientX-drag.x)/innerWidth,-.35,.35);
    wake();
  });
  function endDrag(event,cancel=false){
    if(!drag)return;
    const dx=event.clientX-drag.x,dy=event.clientY-drag.y;drag=null;home.classList.remove('is-dragging');
    if(!cancel&&Math.abs(dx)>50&&Math.abs(dx)>Math.abs(dy))next(dx<0?1:-1);
    wake();
  }
  ...
  home.addEventListener('pointerleave',()=>{pointer=[0,0];wake();});
  window.addEventListener('resize',wake);
  document.addEventListener('visibilitychange',()=>{
    if(document.hidden){if(frame)cancelAnimationFrame(frame);frame=0;last=0;}
    else wake();
  });
  canvas.addEventListener('webglcontextlost',event=>{event.preventDefault();useFallback();});
  motion.addEventListener('change',()=>{
    title.getAnimations({subtree:true}).forEach(animation=>animation.finish());
    if(motion.matches){holdCancels.forEach(cancel=>cancel());originalHold=null;originalSettle=null;art.classList.remove('webgl-ready');if(transition){const done=transition.done;transition=null;done();}}
    else{if(!gl)initGL();wake();}
  });
  window.addEventListener('pagehide',()=>{
    disposed=true;if(frame)cancelAnimationFrame(frame);
    if(gl){images.forEach(asset=>{if(asset.texture)gl.deleteTexture(asset.texture);});gl.deleteBuffer(buffer);gl.deleteProgram(program);}
  },{once:true});
  window.addEventListener('pageshow',event=>{if(event.persisted){disposed=false;images.forEach(asset=>asset.texture=null);gl=null;initGL();wake();}});
  showHome();updateCopy(0,1);initGL();home.classList.add('entering');
