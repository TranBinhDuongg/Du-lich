...
function page(screen){
 ...
 if(nodes['studio-form']){nodes['studio-form'].elements=Object.fromEntries(['destination','days','people','budget'].map(k=>{const el=node();el.value=({destination:'halong',days:3,people:2,budget:3000000})[k];return [k,el]}));nodes['studio-form'].reset=()=>{};}
...
let multiple=page('plan');
multiple.click('add-destination');
multiple.nodes['studio-form'].elements.destination.value='hanoi';
multiple.nodes['studio-form'].elements.destination.change({target:multiple.nodes['studio-form'].elements.destination});
multiple.click('add-destination');
multiple.nodes['studio-form'].elements.destination.value='halong';
multiple.nodes['studio-form'].elements.destination.change({target:multiple.nodes['studio-form'].elements.destination});
multiple.nodes['studio-form'].submit({preventDefault(){}});
assert.deepEqual(multiple.draft().input.destinations,['danang','hanoi']);
